...
- 작업 브랜치: `develop`
- 최신 작업 커밋: `a9e0688` — 클립보드 오류 복구 및 WebKit 회귀 검증 추가
- 이전 커밋: `ac85a77` — 공유 실패 처리·컬렉션 모바일 접근성, `b07e073` — 구버전 확장 인증 API 보강
- 사용자가 최신 작업의 **운영 배포 완료를 알렸다**. 에이전트가 이번 배포의 커밋 SHA, CI 결과, 운영 동작을 직접 확인한 상태는 아니다.
...
- [ ] 배포 대시보드에서 실제 배포 커밋과 빌드 성공 여부 확인. `a9e0688` 또는 이를 포함한 커밋인지 확인한다.
- [ ] Google 로그인·로그아웃, 대시보드·영상 조회 확인.
- [ ] 관리자 계정의 `/admin` 접근과 비관리자 계정의 접근 거부 확인. 거부 확인 버튼은 이전 화면으로 돌아가야 한다.
- [ ] 공유 생성 → 비로그인 열람 → 복사 → 폐기 후 새 요청 차단 확인.
- [ ] 컬렉션 생성·영상 추가/제거·새로고침 유지·삭제 확인. 컬렉션 삭제 후 원본 영상은 보존돼야 한다.
- [ ] 관련 운영 오류 로그 확인. 토큰·세션 쿠키·문서 본문은 기록하지 않는다.
- [ ] 검증 일시, 배포 커밋, 브라우저, 성공/실패, 남은 제한을 `production-validation.md`에 추가한다.
...
아래는 2026-09-13 이전 점검 기록이며 **현재 Vercel 설정을 다시 읽어 확인해야 한다**.
...
- [ ] `ADMIN_UID` 유지. 관리자 권한은 서버 세션 Firebase UID와 이 값 하나의 일치 여부로 판단한다.
...
- [ ] `yarn build:ci`의 `metadataBase` 경고 발생 경로 조사.
- [ ] `src/app/[locale]/layout.tsx`에는 이미 `metadataBase`가 있으므로 단순 중복 추가하지 말고 루트 메타데이터·Open Graph 이미지 경로와 상속 범위 확인.
- [ ] 한국어·영어 페이지의 canonical/Open Graph/Twitter 이미지 URL이 올바른 도메인으로 생성되는지 검증.
- [ ] 수정 후 타입·빌드 검사와 배포 후 공유 미리보기 확인.
